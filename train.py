...
def greedy_mutations(start_tree:t.Tree, ground_truth_tree:t.Tree, train_responses, train_list, validate_list, test_list):
  ...
  prev_train_error = gaps.assess_error(prev_tree, ground_truth_tree, train_list)
  tree_mutation_training_error.append(prev_train_error)

  prev_validation_error = gaps.assess_error(start_tree, ground_truth_tree, validate_list)
  tree_mutation_validation_error.append(prev_validation_error)

  prev_test_error = gaps.assess_error(start_tree, ground_truth_tree, test_list)
  tree_mutation_testing_error.append(prev_test_error)

  num_successful_mutations = 0
  num_unsuccessful_mutations = 0
  while num_successful_mutations < MUTATION_SIZE and num_unsuccessful_mutations <2000:
    curr_tree = prev_tree.mutate()
    curr_tree.train_leaf_prob(train_list, train_responses)
    curr_train_error = gaps.assess_error(curr_tree, ground_truth_tree, train_list)
    curr_validation_error = gaps.assess_error(curr_tree, ground_truth_tree, validate_list)
    curr_test_error = gaps.assess_error(curr_tree, ground_truth_tree, test_list)
...
def chain_mutations(start_tree:t.Tree, ground_truth_tree:t.Tree, train_responses, train_list, validate_list, test_list):
  ...
  ground_truth_probs_trn = ground_truth_tree.run_tree_gaps(train_list, leaf_prob=True)
  ground_truth_probs_val = ground_truth_tree.run_tree_gaps(validate_list, leaf_prob=True)
  ground_truth_probs_tst = ground_truth_tree.run_tree_gaps(test_list, leaf_prob=True)

  prev_tree.train_leaf_prob(train_list, train_responses)

  prev_prob_trn = prev_tree.run_tree_gaps(train_list, leaf_prob=True)
  prev_train_error = gaps.assess_error(prev_tree, ground_truth_tree, train_list)
  tree_mutation_training_error.append(prev_train_error)

  prev_prob_val = prev_tree.run_tree_gaps(validate_list, leaf_prob=True)
  prev_validation_error = gaps.assess_error(start_tree, ground_truth_tree, validate_list)
  tree_mutation_validation_error.append(prev_validation_error)

  prev_prob_tst = prev_tree.run_tree_gaps(test_list, leaf_prob=True)
  prev_test_error = gaps.assess_error(start_tree, ground_truth_tree, test_list)
  tree_mutation_testing_error.append(prev_test_error)
...
def ensemble(num_trees, ground_truth, train_responses, train_list, validate_list, test_list):
  ...
  tree_outputs = []  
  for tree in forest:
    mutated_tree = greedy_mutations(tree, ground_truth, train_responses, train_list, validate_list, test_list)
    tree_outputs.append(mutated_tree)
  for tree in tree_outputs:
    tree.print2D(tree.root.child)
